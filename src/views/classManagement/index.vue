          ...
          <div class="schedule-class-container">
            <el-collapse accordion @change="classSlideChange">
              <el-collapse-item v-for="item in classList" :key="item.groupId">
                ...
                >
                  <div
                    v-for="(part, partIndex) in item.classesList"
                    :key="`${item.groupId}-${part.id}-${partIndex}`"
          ...
          <div class="schedule-table-body">
            ...
            >
              ...
              >
                <!-- 课表循环 -->
                <div
                  v-for="(classItem, classIndex) in item.classSchedule"
                  :key="`${item.commonDate}-${classItem.id}-${classIndex}`"
                ...
                <div
                  ...
                  :key="`${item.commonDate}-${activityItem.activityId}-${activityIndex}`"
  ...
  methods: {
            ...
            this.currentWeek = this.currentWeek.map((item) => {
              ...
              res.result.forEach((part) => {
                if (item.commonDate === part.dataDate) {
                  // 处理 activityList，确保唯一性
                  const newActivityList = (part.activityOverviewList || []).map(
                    (i) => {
                      let completion = "";
                  ...
                  const existingIds = new Set(
                    activityList.map((a) => a.activityId)
                  );
                  const uniqueNewActivities = newActivityList.filter(
                    (a) => !existingIds.has(a.activityId)
                  );
                  activityList = [...activityList, ...uniqueNewActivities];

                  // 处理 classSchedule
                  const newClassSchedule = (part.classScheduleVoList || [])
                    .map((i) => {
                  ...
                  const existingClassIds = new Set(
                    classSchedule.map((c) => c.id)
                  );
                  const uniqueNewClasses = newClassSchedule.filter(
                    (c) => !existingClassIds.has(c.id)
                  );
                  classSchedule = [...classSchedule, ...uniqueNewClasses];
                }
              });
              return {
                ...item,
    ...
    async AddScheduleClass(data) {
      ...
      data.classesJson = JSON.stringify({
        ...JSON.parse(originalClassesJson),
        duration: secondsToHHMMSS(res.result.time),
        distance: res.result.distance + "km",
        sth: res.result.sth,
      });
      submitData({
