  ...
  computed: {
    ...
    filteredAthleticData() {
      ...
      return this.athleticData.filter(group => {
        return group.label.toLowerCase().includes(this.searchInput.toLowerCase()) ||
               (group.children && group.children.some(member =>
                 member.label.toLowerCase().includes(this.searchInput.toLowerCase())
               ))
      })
    },
    // 可用的目标分组（排除当前分组）
    availableGroups() {
      return this.athleticData.filter(group => group.id !== this.currentGroup.id)
    }
  },
  watch: {
    teamId: {
