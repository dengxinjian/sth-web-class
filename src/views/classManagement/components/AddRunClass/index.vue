<template>
    ...
    <div class="basic-info">
        ...
        <div class="basic-info-item">
            <div>
                <span>模式选择：</span>
                <el-select v-model="classInfo.mode" class="pill-select short" @change="calculateTimeline">
                     ...
                     <div v-for="(part, idx) in item.sections" :key="idx + '' + index" class="stage-config">
                        <div class="config-row">
                            <div class="config-item">
                                ...
                                <el-input v-model="part.title" placeholder="请输入阶段名称" size="small" @change="calculateTimeline" />
                            </div>
                            <div class="config-item">
                                <span class="label required">段落模式选择</span>
                                <el-select v-model="part.stageMode" placeholder="请选择" size="small">
                            ...
                            <div class="config-item">
                                ...
                                <el-select v-model="part.capacity" placeholder="请选择" size="small" @change="handleTargetChange(index, idx)">
                        ...
                        <div class="config-row">
                            ...
                            <div v-if="classInfo.mode === 1 && part.range === 'range'" class="config-item">
                                ...
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdSpeedRange[0]" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdSpeedRange[1]" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">%阈值配速</span>
                            </div>
                            <div v-else-if="classInfo.mode === 1 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdSpeed" placeholder="80" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">%阈值配速</span>
                            </div>
                            <div v-if="classInfo.mode === 2 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRateRange[0]" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRateRange[1]" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">%阈值心率</span>
                            </div>
                            <div v-else-if="classInfo.mode === 2 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRate" placeholder="80" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">%阈值心率</span>
                            </div>
                            <div v-if="classInfo.mode === 3 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-time-picker value-format="mm:ss" v-model="part.targetSpeedRange[0]" format="mm:ss" size="small" popper-class="hide-hour" :style="{ width: '100px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">~</span>
                                <el-time-picker v-model="part.targetSpeedRange[1]" value-format="mm:ss" format="mm:ss" size="small" popper-class="hide-hour" :style="{ width: '100px' }" @change="handleTargetChange(index, idx)"></el-time-picker>
                                <span class="unit">min/km</span>
                            </div>
                            <div v-else-if="classInfo.mode === 3 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-time-picker v-model="part.targetSpeed" value-format="mm:ss" format="mm:ss" size="small" popper-class="hide-hour" :style="{ width: '100px' }" @change="handleTargetChange(index, idx)"></el-time-picker>
                                <span class="unit">min/km</span>
                            </div>
                            <div v-if="classInfo.mode === 4 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="1" :step-strictly="true" v-model="part.targetHeartRateRange[0]" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="1" :step-strictly="true" v-model="part.targetHeartRateRange[1]" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">bpm</span>
                            </div>
                            <div v-else-if="classInfo.mode === 4 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="1" :step-strictly="true" v-model="part.targetHeartRate" placeholder="80" size="small" :style="{ width: '80px' }" @change="handleTargetChange(index, idx)" />
                                <span class="unit">bpm</span>
                            </div>
                            <div v-if="part.hasCadence" class="config-item">
                                <el-input-number :controls="false" :step="1" :step-strictly="true" v-model="part.cadence[0]" placeholder="80" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="1" :step-strictly="true" v-model="part.cadence[1]" placeholder="80" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                        ...
                        <div class="btn-row">
                            <div class="stage-actions">
                                <el-button class="action-btn-add action-btn" type="text" size="mini" circle @click="handleAddSection(index)">
                                    <i class="el-icon-plus"></i>增加段落
                                </el-button>
                                <el-button class="action-btn-remove action-btn" type="text" size="mini" circle @click="handleRemoveSection(index, idx)">
                                    <i class="el-icon-close"></i>移除段落
                                </el-button>
                                <el-button v-if="!part.hasCadence" type="text" size="mini" class="add-cadence-btn" @click="handleAddCadence(index, idx)">
  ...
  methods: {
    ...
    submitNewClass(flag) {
      submitData({
        ...
        sportType: 'RUN',
        classesJson: JSON.stringify({...this.classInfo, timeline: this.timeline, maxIntensity: this.maxIntensity})
      }).then(res => {
        if (res.success) {
