                ...
                <div class="section-header">
                 ...
                 <div v-for="(item, index) in classInfo.stages" :key="index" class="stage-section">
                    <div class="stage-header">
                        ...
                        <el-input-number :controls="false" :step="1" :step-strictly="true" v-model="item.times" size="small" disabled />
                    </div>
                     <div v-for="(part, idx) in item.sections" :key="idx + '' + index" class="stage-config">
                        <div class="config-row">
                            <div class="config-item">
                                <span class="label">阶段名</span>
                                <el-input v-model="part.title" placeholder="阶段名称" size="small" disabled />
                            </div>
                            <div class="config-item">
                            ...
                            <div class="config-item">
                                ...
                                <el-select v-model="part.capacity" placeholder="请选择" size="small" disabled>
                                    <el-option label="运动时间" value="time" />
                                </el-select>
                            </div>
                            <div class="config-item">
                                <span class="label">范围选择</span>
                                <el-select v-model="part.range" placeholder="请选择" size="small" disabled>
                        ...
                        <div class="config-row">
                            <div class="config-item">
                                ...
                                <el-time-picker v-model="part.target" value-format="HH:mm:ss" placeholder="请选择" size="small" disabled style="max-width: 140px;">
                                </el-time-picker>
                            </div>
                            <div v-if="classInfo.mode === 1 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdFtpRange[0]" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdFtpRange[1]" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">%阈值功率</span>
                            </div>
                            <div v-else-if="classInfo.mode === 1 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdFtp" placeholder="80" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">%阈值功率</span>
                            </div>
                            <div v-if="classInfo.mode === 2 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRateRange[0]" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRateRange[1]" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">%阈值心率</span>
                            </div>
                            <div v-else-if="classInfo.mode === 2 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRate" placeholder="80" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">%阈值心率</span>
                            </div>
                            <div v-if="classInfo.mode === 3 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetFtpRange[0]" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetFtpRange[1]" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">w</span>
                            </div>
                            <div v-else-if="classInfo.mode === 3 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetFtp" placeholder="80" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">w</span>
                            </div>
                            <div v-if="classInfo.mode === 4 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetHeartRateRange[0]" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetHeartRateRange[1]" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">bpm</span>
                            </div>
                            <div v-else-if="classInfo.mode === 4 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetHeartRate" placeholder="80" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">bpm</span>
                            </div>
                            <div v-if="part.hasCadence" class="config-item">
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.cadence[0]" placeholder="80" size="small" :style="{ width: '80px' }" disabled />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.cadence[1]" placeholder="80" size="small" :style="{ width: '80px' }" disabled />
                        ...
                        <div class="btn-row">
                            <div class="stage-actions">
                                <el-button class="action-btn-add action-btn" type="text" size="mini" circle disabled>
                                    <i class="el-icon-plus"></i>增加段落
                                </el-button>
                                <el-button class="action-btn-remove action-btn" type="text" size="mini" circle disabled>
                                    <i class="el-icon-close"></i>移除段落
                                </el-button>
                                <el-button v-if="!part.hasCadence" type="text" size="mini" class="add-cadence-btn" disabled>
