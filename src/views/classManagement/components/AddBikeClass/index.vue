<template>
    ...
    >
    <span slot="title">{{ type === 'add' ? '新建骑行课程' : '编辑骑行课程' }}</span>
    ...
    <div class="time-line" id="timeLineDrag">
        <div v-for="(item, index) in timeline" :key="index" class="time-stage" :style="{flex: item.duration, minWidth: 0}">
            ...
            <div class="time-stage-title">{{ item.stageTimeline.length > 1 ? '骑行' : item.stageTimeline[0].title }}</div>
                    ...
                    <div class="phase-item">
                        <div v-for="(item, index) in classInfo.stages" :key="index">
                            <div v-for="(part, idx) in item.sections" :key="idx">
                                ...
                                <div v-if="classInfo.mode === 2 && part.range === 'range'">{{ part.target }} @ {{ part.thresholdHeartRateRange[0] }}~ {{ part.thresholdHeartRateRange[1] }}% 阈值心率 <span v-if="part.hasCadence">踏频{{ part.cadence[0] }}~{{ part.cadence[1] }}</span></div>
                                <div v-if="classInfo.mode === 1 && part.range === 'target'">{{ part.target }} @ {{ part.thresholdFtp }}% 阈值功率 <span v-if="part.hasCadence">踏频{{ part.cadence[0] }}~{{ part.cadence[1] }}</span></div>
                                <div v-if="classInfo.mode === 2 && part.range === 'target'">{{ part.target }} @ {{ part.thresholdHeartRate }}% 阈值心率 <span v-if="part.hasCadence">踏频{{ part.cadence[0] }}~{{ part.cadence[1] }}</span></div>
                                <div v-if="classInfo.mode === 3 && part.range === 'range'">{{ part.target }} @ {{ part.targetFtpRange[0] }}~ {{ part.targetFtpRange[1] }}w <span v-if="part.hasCadence">踏频{{ part.cadence[0] }}~{{ part.cadence[1] }}</span></div>
                                <div v-if="classInfo.mode === 4 && part.range === 'range'">{{ part.target }} @ {{ part.targetHeartRateRange[0] }}~ {{ part.targetHeartRateRange[1] }}bpm <span v-if="part.hasCadence">踏频{{ part.cadence[0] }}~{{ part.cadence[1] }}</span></div>
                                <div v-if="classInfo.mode === 3 && part.range === 'target'">{{ part.target }} @ {{ part.targetFtp }}w <span v-if="part.hasCadence">踏频{{ part.cadence[0] }}~{{ part.cadence[1] }}</span></div>
                                <div v-if="classInfo.mode === 4 && part.range === 'target'">{{ part.target }} @ {{ part.targetHeartRate }}bpm <span v-if="part.hasCadence">踏频{{ part.cadence[0] }}~{{ part.cadence[1] }}</span></div>
                            </div>
                     ...
                     <div v-for="(part, idx) in item.sections" :key="idx + '' + index" class="stage-config">
                        <div class="config-row">
                            ...
                            <div class="config-item">
                                ...
                                <el-select v-model="part.capacity" placeholder="请选择" size="small">
                        ...
                        <div class="config-row">
                            ...
                            <div v-if="classInfo.mode === 1 && part.range === 'range'" class="config-item">
                                ...
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdFtpRange[0]" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdFtpRange[1]" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">%阈值功率</span>
                            </div>
                            <div v-else-if="classInfo.mode === 1 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdFtp" placeholder="80" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">%阈值功率</span>
                            </div>
                            <div v-if="classInfo.mode === 2 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRateRange[0]" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRateRange[1]" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">%阈值心率</span>
                            </div>
                            <div v-else-if="classInfo.mode === 2 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.thresholdHeartRate" placeholder="80" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">%阈值心率</span>
                            </div>
                            <div v-if="classInfo.mode === 3 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetFtpRange[0]" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetFtpRange[1]" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">w</span>
                            </div>
                            <div v-else-if="classInfo.mode === 3 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetFtp" placeholder="80" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">w</span>
                            </div>
                            <div v-if="classInfo.mode === 4 && part.range === 'range'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetHeartRateRange[0]" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetHeartRateRange[1]" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">bpm</span>
                            </div>
                            <div v-else-if="classInfo.mode === 4 && part.range === 'target'" class="config-item">
                                <span class="label">运动值</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.targetHeartRate" placeholder="80" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">bpm</span>
                            </div>
                            <div v-if="part.hasCadence" class="config-item">
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.cadence[0]" placeholder="80" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
                                <span class="unit">~</span>
                                <el-input-number :controls="false" :step="0.1" :step-strictly="true" v-model="part.cadence[1]" placeholder="80" size="small" :style="{ width: '80px' }" @change="calculateTimeline" />
...
export default {
  ...
  methods: {
    ...
    handleTargetChange(index, sectionIndex) {
      this.classInfo.stages[index].sections[sectionIndex].targetSeconds = this.translateTarget(this.classInfo.stages[index].sections[sectionIndex].target)
      this.calculateTimeline()
    },
      ...
      switch (mode) {
        case 1:
          if (section.range === 'range') {
            return (section.thresholdFtpRange[0] + section.thresholdFtpRange[1]) / 2
          } else {
